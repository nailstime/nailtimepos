-- Add salary and commission to the allowed expense categories.
-- No data migration needed — these are new categories with no legacy data.
...
begin
  ...
  if p_kind = 'expense' and btrim(coalesce(p_category,'')) not in
    ('water', 'electricity', 'internet', 'phone',
     'product_cost', 'service_cost', 'salary', 'commission_expense',
     'regular_expense', 'other_expense', 'interest_fee', 'refund') then
