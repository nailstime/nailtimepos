...
export default function LiffBooking() {
  ...
  async function api(action, payload = {}) {
    ...
    if (invokeError || data?.error) {
      ...
      if (!message && invokeError?.context?.clone) {
        try { message = (await invokeError.context.clone().json())?.error } catch { /* ignore */ }
      }
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        const { data, error: invokeError } = await supabase.functions.invoke('booking-liff', { body: { action: 'services', id_token: idToken } })
  ...
  async function selectDate(nextDate) {
    ...
    try {
      setSlots(await api('slots', { date: nextDate, service_id: service.id }))
      setState('slot')
  ...
  async function confirmBooking() {
    ...
    try {
      const result = await api('book', { service_id: service.id, slot_id: slot.id, date, guest_name: profile?.displayName || '' })
      setBooking(result); setState('done')
    } catch (err) { setError(String(err.message || err)) } finally { setBusy(false) }
  }
